...
void startAnimationChangeMarkers() {
  _startPreviousMarkersFadeAnimation(onFinish: () => _startCurrentMarkersFadeAnimation(),);
}
  ...
  void _startPreviousMarkersFadeAnimation({VoidCallback? onFinish}) {
    ...
    _fadeStreamSubscription = _generateFadeValues(
      ...
      difference: 0.1,
      durationPerInteraction:Duration(milliseconds:  duration.inMilliseconds~/ (1/0.1))
    ).listen((event) {
      if (isClosed) return;
      if (event > 0) {
        log(event.toString());
  ...
  Stream<double> _generateFadeValues({
    ...
    assert(initialValue >= 0 && initialValue<=1, 'Initial value must be greater than 0 and lower than 1');
    assert(finalValue >= 0 && finalValue <=1, 'Initial value must be greater than 0 and lower than 1');
    yield initialValue;
    switch (operation) {
